  ...
  if (role === "volunteer" && isUpcoming) {
    return (
      <div className="flex flex-col gap-3 p-5 bg-[#FACC15]/8 rounded-2xl border border-[#FACC15]/25">
        ...
        <button className="w-full py-3 rounded-xl bg-[#FACC15] text-black font-['Syne'] font-bold text-sm hover:bg-yellow-300 transition-all duration-200 shadow-lg shadow-[#FACC15]/20">
...
export default function EventDetailPage() {
  ...
  // Demo role — replace with real auth context
  ...
  if (!event) {
    return (
      <div className="bg-[#0A0A0A] min-h-screen flex flex-col items-center justify-center gap-5 px-6">
        ...
        <Link to="/events" className="text-[#FACC15] text-sm hover:underline">
          ← Back to Events
        </Link>
      </div>
    );
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-10 pt-28 pb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-start">
          ...
          >
            ...
            {/* Meta info */}
            <div
              className="flex flex-col gap-3 p-5 bg-[#111] rounded-2xl"
              style={{ boxShadow: "0 0 0 1px rgba(255,255,255,0.07)" }}
            >
            ...
            {!isUpcoming && (
              <div
                className="p-5 bg-[#FACC15]/5 rounded-2xl border border-[#FACC15]/15"
              >
                <p className="text-[#FACC15] text-[10px] font-bold tracking-widest uppercase mb-2">
                  Impact Summary
                </p>
                <p className="text-white/55 text-xs leading-relaxed">
                  This event was successfully conducted by Voice For Welfare volunteers and community members,
                  creating measurable change in the lives of those involved. Detailed impact reports are
                  available in our annual publication.
                </p>
              </div>
            )}

            {/* Role-based CTAs */}
            <EventCTAs event={event} role={role} />
          </motion.div>
        </div>
      </div>
    </div>
