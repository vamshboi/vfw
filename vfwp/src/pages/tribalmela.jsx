...
const PRODUCTS = [
  {
    ...
    badge: "Bestseller",
    emoji: "🖼️",
    description: "Hand-painted Warli art depicting village life, celebrations, and nature in traditional white on terracotta.",
    image: null,
    rating: 4.9,
    reviews: 42,
    inStock: true,
  },
  {
    ...
    badge: "Handcrafted",
    emoji: "🏺",
    description: "Lost-wax cast brass figurine using 5000-year-old Dhokra craft tradition from Chhattisgarh.",
    image: null,
    rating: 4.8,
    reviews: 28,
    inStock: true,
  ...
  {
    id: 3,
    name: "Gond Painting — Forest Spirit",
    artist: "Durgabai Vyam",
    tribe: "Gond",
    price: 4200,
    originalPrice: 5000,
    category: "Paintings",
    badge: "Award Winning",
    emoji: "🌿",
    description: "Vibrant Gond painting with intricate dot patterns depicting the forest deity and tribal folklore.",
    image: null,
    rating: 5.0,
    reviews: 67,
    inStock: true,
  },
  {
    ...
    badge: "New Arrival",
    emoji: "💍",
    description: "Handcrafted oxidized silver choker with traditional Lambadi mirror-work and thread embroidery.",
    image: null,
    rating: 4.7,
    reviews: 19,
    inStock: true,
  ...
  {
    id: 5,
    name: "Bamboo Weave Storage Basket",
    artist: "Suresh Majhi",
    tribe: "Santhali",
    price: 680,
    originalPrice: null,
    ...
    badge: null,
    emoji: "🧺",
    description: "Eco-friendly handwoven bamboo basket using techniques passed down through Santhali generations.",
    image: null,
    rating: 4.6,
    reviews: 35,
    inStock: true,
  },
  {
    ...
    emoji: "📜",
    description: "Traditional cloth-based scroll painting narrating Jagannath mythology in vivid natural pigments.",
    image: null,
    rating: 4.9,
    reviews: 53,
    inStock: false,
  },
  {
    ...
    badge: null,
    emoji: "🐟",
    description: "Auspicious fish motif in classic Madhubani style — a traditional wedding gift in Bihar.",
    image: null,
    rating: 4.8,
    reviews: 31,
    inStock: true,
  ...
  {
    id: 8,
    name: "Tribal Terracotta Horse",
    artist: "Mohan Kumbhar",
    tribe: "Kutch",
    price: 920,
    originalPrice: null,
    category: "Artifacts",
    badge: null,
    emoji: "🐴",
    description: "Hand-molded and kiln-fired terracotta horse — a sacred offering piece used in tribal rituals.",
    image: null,
    rating: 4.5,
    reviews: 14,
    inStock: true,
  },
  {
    ...
    badge: "Popular",
    emoji: "📿",
    description: "Handstrung colorful glass bead bracelet in traditional Bhil geometric patterns from Rajasthan.",
    image: null,
    rating: 4.7,
    reviews: 88,
    inStock: true,
    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg key={s} viewBox="0 0 12 12" fill="none" className="w-3 h-3">
          <path
            d="M6 1l1.236 2.504L10 3.882l-2 1.95.472 2.75L6 7.25 3.528 8.582 4 5.832 2 3.882l2.764-.378L6 1z"
            fill={s <= Math.round(rating) ? "#FACC15" : "rgba(255,255,255,0.1)"}
          />
        </svg>
      ))}
    </div>
...
function ProductCard({ product, index, onAddToCart, onQuickView }) {
  ...
  const gradients = {
    Paintings: "from-amber-900/30 to-[#141414]",
    Handicrafts: "from-orange-900/20 to-[#141414]",
    Jewelry: "from-yellow-900/20 to-[#141414]",
    Artifacts: "from-purple-900/20 to-[#141414]",
  };
  ...
  return (
    <motion.div
      ...
      className="group relative bg-[#111111] rounded-2xl overflow-hidden flex flex-col"
      style={{ boxShadow: "0 0 0 1px rgba(255,255,255,0.07)" }}
    >
      {/* Hover glow */}
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-2xl z-0"
        style={{ background: "radial-gradient(ellipse at 50% 0%,rgba(250,204,21,0.07) 0%,transparent 70%)" }} />
      ...
      <div className="relative w-full aspect-square overflow-hidden shrink-0">
        <div className={`w-full h-full bg-gradient-to-br ${gradients[product.category] || "from-[#1e1e1e] to-[#141414]"} flex items-center justify-center`}>
        ...
        {product.badge && (
          <div className="absolute top-3 left-3 z-10">
            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full tracking-wider ${BADGE_STYLES[product.badge] || "bg-white/10 text-white/60"}`}>
              {product.badge}
            </span>
          </div>
        )}
        ...
        {!product.inStock && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10">
            <span className="text-white/60 text-xs font-semibold tracking-widest uppercase bg-black/50 px-3 py-1.5 rounded-full border border-white/15">
              Out of Stock
            </span>
          </div>
        )}
        ...
        <button
          ...
          className="absolute bottom-3 right-3 z-10 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 w-8 h-8 rounded-full bg-black/70 hover:bg-black/90 flex items-center justify-center backdrop-blur-sm"
        >
          <svg viewBox="0 0 16 16" fill="none" className="w-3.5 h-3.5 text-white">
            <circle cx="8" cy="8" r="3" stroke="currentColor" strokeWidth="1.5" />
            <path d="M1.5 8C3 4.5 5.5 2.5 8 2.5s5 2 6.5 5.5C13 11.5 10.5 13.5 8 13.5S3 11.5 1.5 8z" stroke="currentColor" strokeWidth="1.5" />
          </svg>
        </button>
      </div>
      ...
      <div className="p-4 flex flex-col gap-2.5 flex-1 relative z-10">
        ...
        <h3 className="font-['Syne'] text-white font-semibold text-sm leading-snug group-hover:text-[#FACC15] transition-colors duration-300 line-clamp-1">
          {product.name}
        </h3>
        ...
        <div className="pt-2.5 border-t border-white/5 flex items-center justify-between gap-3 mt-1">
          <div className="flex flex-col">
            ...
            {product.originalPrice && (
              <span className="text-white/30 text-[10px] line-through">₹{product.originalPrice.toLocaleString()}</span>
            )}
          </div>
          <motion.button
            whileTap={{ scale: 0.93 }}
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-['Syne'] font-bold transition-all duration-300 ${
              added
          ...
          >
            {added ? (
              ...
              <>
                <svg viewBox="0 0 16 16" fill="none" className="w-3.5 h-3.5">
                  <path d="M1 1h2l2.5 8h7l1.5-5H5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
...
// ─── Cart Drawer ──────────────────────────────────────────────────────────────
function CartDrawer({ cart, total, count, onClose, onRemove, onUpdateQty }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <motion.div
        ...
        className="relative ml-auto w-full max-w-md h-full bg-[#111111] flex flex-col"
      ...
      >
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <motion.div
                ...
                exit={{ opacity: 0, x: 20 }}
                className="flex items-center gap-4 p-3 bg-[#1a1a1a] rounded-xl"
                style={{ boxShadow: "0 0 0 1px rgba(255,255,255,0.06)" }}
              >
                <div className="w-14 h-14 rounded-lg bg-[#222] flex items-center justify-center shrink-0 text-2xl">
                  {item.emoji}
                </div>
        ...
        {cart.length > 0 && (
          <div className="px-6 py-5 border-t border-white/8 flex flex-col gap-3">
            ...
            <button className="w-full py-3.5 rounded-xl bg-[#FACC15] text-black font-['Syne'] font-bold text-sm hover:bg-yellow-300 transition-all duration-200 shadow-lg shadow-[#FACC15]/25">
              Proceed to Checkout →
            </button>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full max-w-2xl bg-[#111111] rounded-2xl overflow-hidden"
        style={{ boxShadow: "0 0 0 1px rgba(255,255,255,0.08), 0 40px 100px rgba(0,0,0,0.7)" }}
      >
        <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[#FACC15] to-transparent" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2">
          {/* Left: visual */}
          <div className="aspect-square sm:aspect-auto bg-gradient-to-br from-amber-900/30 to-[#141414] flex items-center justify-center">
          ...
          <div className="p-7 flex flex-col gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                <span className="text-[#FACC15]/70 text-[10px] font-bold tracking-widest uppercase">{product.tribe} Tribe</span>
                {product.badge && (
                  <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full tracking-wider ${BADGE_STYLES[product.badge] || "bg-white/10 text-white/60"}`}>
                    {product.badge}
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center gap-2">
              <Stars rating={product.rating} />
              <span className="text-white/40 text-xs">{product.rating} ({product.reviews} reviews)</span>
            </div>

            <p className="text-white/55 text-sm leading-relaxed">{product.description}</p>

            <div className="p-4 bg-[#1a1a1a] rounded-xl flex items-center justify-between"
              style={{ boxShadow: "0 0 0 1px rgba(255,255,255,0.06)" }}>
              <div>
              ...
              {product.originalPrice && (
                <span className="text-emerald-400 text-xs font-bold bg-emerald-500/15 px-2.5 py-1 rounded-full">
            ...
            <div className="p-3 bg-[#FACC15]/5 rounded-xl border border-[#FACC15]/15 flex items-start gap-2.5">
              ...
              <p className="text-white/50 text-xs leading-relaxed">
                Your purchase directly supports <strong className="text-white/70">{product.artist}</strong> and their family. 100% of proceeds go to the artisan.
              </p>
            </div>
            ...
            <motion.button
              ...
              onClick={handleAdd}
              disabled={!product.inStock}
              className={`w-full py-3.5 rounded-xl font-['Syne'] font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 ${
                added ? "bg-emerald-500 text-white" : product.inStock
                  ? "bg-[#FACC15] text-black hover:bg-yellow-300 shadow-lg shadow-[#FACC15]/20"
                  : "bg-white/5 text-white/25 cursor-not-allowed"
              }`}
            >
...
function ImpactBanner() {
  const stats = [
    { value: "200+", label: "Artisans Supported", icon: "👨‍🎨" },

    { value: "15+", label: "Tribal Crafts", icon: "🏺" },
    { value: "2", label: "States Covered", icon: "🗺️" },
  ];
  ...
  return (
    <motion.div
      ...
      className="relative rounded-2xl overflow-hidden p-8 mb-12"
      style={{
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#FACC15]/20 to-transparent" />
    ...
    <div className="bg-[#0A0A0A] min-h-screen">
      ...
      >
        <svg viewBox="0 0 20 20" fill="none" className="w-5 h-5">
          <path d="M1 1h3l2.5 9.5h9l2-6H6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
        ...
        {count > 0 && (
          <motion.span
            ...
            initial={{ scale: 0.5 }}
            animate={{ scale: 1 }}
            className="bg-black text-[#FACC15] text-[10px] font-extrabold w-5 h-5 rounded-full flex items-center justify-center"
          >
            {count}
          </motion.span>
        )}
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-10 pt-28 pb-24">
        ...
        <motion.div
          ...
          className="flex flex-col items-center text-center mb-14 gap-5"
        >
          ...
          <p className="text-white/45 text-base max-w-xl leading-relaxed">
            Every handcrafted piece carries centuries of tribal wisdom. Your purchase directly empowers the artisan who made it.
          </p>

          {/* Search */}
          <div className="relative w-full max-w-md mt-2">
            <svg viewBox="0 0 20 20" fill="none" className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30">
              <circle cx="9" cy="9" r="6" stroke="currentColor" strokeWidth="1.6" />
              <path d="M13.5 13.5L17 17" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
            </svg>
            <input
              type="text"
              placeholder="Search by art, artist, or tribe…"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-[#111] border border-white/10 rounded-xl pl-11 pr-4 py-3 text-white text-sm placeholder-white/25 focus:outline-none focus:border-[#FACC15]/40 focus:ring-1 focus:ring-[#FACC15]/15 transition-all duration-200"
            />
            {search && (
        ...
        >
          {/* Category pills */}
          <div className="flex items-center gap-1.5 p-1 bg-[#111] rounded-xl border border-white/8 flex-wrap">
            {CATEGORIES.map((cat) => (
              <button
                key={cat}
          ...
          <select
            ...
            className="bg-[#111] border border-white/10 rounded-xl px-4 py-2.5 text-white/60 text-xs font-semibold focus:outline-none focus:border-[#FACC15]/40 transition-all duration-200 cursor-pointer"
          >
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          {(search || activeCategory !== "All") && (
            <button
              ...
              className="text-[#FACC15]/60 text-xs hover:text-[#FACC15] transition-colors"
            >
              Clear filters ×
            </button>
          )}
        ...
        {filtered.length > 0 ? (
          ...
          >
            ...
            <button onClick={() => { setSearch(""); setActiveCategory("All"); }} className="text-[#FACC15]/60 text-xs hover:text-[#FACC15] transition-colors">
              Browse all products →
            </button>
          </motion.div>
        )}
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="mt-20 rounded-2xl p-10 flex flex-col sm:flex-row items-center justify-between gap-6 relative overflow-hidden"
          style={{
        ...
        >
          ...
          <div className="absolute -right-20 -bottom-20 w-60 h-60 rounded-full bg-[#FACC15]/4 blur-3xl pointer-events-none" />
          ...
          <div className="flex flex-col sm:flex-row gap-3 relative z-10">
            <button className="px-6 py-3 rounded-xl bg-[#FACC15] text-black font-['Syne'] font-bold text-sm hover:bg-yellow-300 transition-all duration-200 shadow-lg shadow-[#FACC15]/20 whitespace-nowrap">
              Register as Artisan →
            </button>
            <button className="px-6 py-3 rounded-xl border border-white/10 text-white/60 font-medium text-sm hover:border-white/20 hover:text-white/80 transition-all duration-200 whitespace-nowrap">
              Learn More
            </button>
          </div>
        </motion.div>
